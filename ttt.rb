...
def throwDices(myPoint, hisPoint)
    myPoint = myPoint
    hisPoint = hisPoint
    diceStrings = ['| * |', '| : |', '|...|', '|: :|','|:.:|','|:::|']
    myDice = rand(0..5)
    ...
    myPoint, hisPoint = judge(myDice, hisDice, myPoint, hisPoint)
    return myPoint, hisPoint
end

def judge(myDice, hisDice, myPoint, hisPoint)
    if myDice > hisDice
        puts "+ 10P ゲット！"
        myPoint += 10
        hisPoint -= 10
...
def main()
    ...
    until @myPoint == 0 or @myPoint == 100
        ...
        puts "エンターでゲームスタート！"
        gets
        myPoint, hisPoint = throwDices(myPoint, hisPoint)
        puts ""
        puts "現在の得点"
